...
| First Seen / Last Activity | ✅ | `GlobalSequenceDetails.CREATION_TIME` (per-entity, FK'd via `ENTITY_ID`) + ES `min/max(_zl_timestamp)` filtered by entity. Note: `GlobalSequenceDetails.CREATION_TIME` is per-entity-sequence, not per-anomaly, so it's safer than ES min for "first ever seen by the platform" but still not equal to "first anomaly". | Raw DB + ES agg | — |
