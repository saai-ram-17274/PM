...
| Account Status (with recommendation) | 🟡 both | Stored: `APFDiscADUserDetails.ACCOUNT_STATUS` (BOOLEAN, derived from `userAccountControl` flag bits — see `APFADUserAttributes.xml` priority 18) + raw `USER_ACCOUNT_CONTROL`. Entra: `APFDiscAADUserDetails.ACCOUNT_ENABLED` + raw `USER_ACCOUNT_CONTROL`. The "Recommended: Disable" suffix is product-side business logic, not a stored column. | Direct read + flag decoder + business rule | 🤖✚ AI generates the **recommendation text** ("Disable" / "Force password change") from current risk + attack chain |
