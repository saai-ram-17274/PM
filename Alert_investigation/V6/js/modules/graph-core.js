...
function makeNodeDraggable(g, circle, iconEl, labelEl, nodeId) {
  ...
  function onPointerMove(e) {
    ...
    const svg = document.getElementById('graphSvg');
    // Helper: reposition the edge-info-btn (relation icon) to the line midpoint
    const moveEdgeIcon = (src, tgt, x1, y1, x2, y2) => {
      const btn = svg.querySelector(`g.edge-info-btn[data-source="${src}"][data-target="${tgt}"]`);
      if (!btn) return;
      const mx = (x1 + x2) / 2;
      const my = (y1 + y2) / 2;
      btn.querySelectorAll('circle').forEach(c => { c.setAttribute('cx', mx); c.setAttribute('cy', my); });
      btn.querySelectorAll('text').forEach(t => { t.setAttribute('x', mx); t.setAttribute('y', my + 4); });
    };
    svg.querySelectorAll('line[data-source="' + nodeId + '"]').forEach(edge => {
      ...
      if (tgt) moveEdgeIcon(nodeId, tgt, newCx, newCy, x2, y2);
    });
    svg.querySelectorAll('line[data-target="' + nodeId + '"]').forEach(edge => {
      ...
      if (src) moveEdgeIcon(src, nodeId, x1, y1, newCx, newCy);
    });
  }
...
function initGraphPan() {
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    if (e.target.closest('.graph-node, .graph-zoom, .graph-canvas-toolbar, .gcb-menu, .zia-float, .graph-ctx, .edge-info-btn, button, select')) return;
    graphPanning = true;
